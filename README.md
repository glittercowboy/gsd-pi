...
Both terminals operate against the same project-root GSD database and `.gsd/` projections. Your decisions in terminal 2 are picked up automatically at the next phase boundary — no need to stop auto mode.
...
**Multi-session orchestration** — headless mode uses DB-backed auto-mode state plus file-based IPC in `.gsd/parallel/` for coordinating multiple GSD workers across milestones. Build orchestrators that spawn, monitor, and budget-cap a fleet of GSD workers.
