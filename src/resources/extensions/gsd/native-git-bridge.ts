...
function runBoundedSymlinkFallbackAddAll(basePath: string): void {
...
function fallbackStageWithSymlinkedDotGsd(basePath: string): void {
  if (isDotGsdIgnored(basePath)) {
    runBoundedSymlinkFallbackAddAll(basePath);
    return;
  }
...
export function nativeAddAllWithExclusions(basePath: string, exclusions: readonly string[]): void {
  ...
  try {
    ...
    // either adds `.gsd` to `.gitignore` and retries a bounded `git add -A`,
    // or stages real files explicitly when `git.manage_gitignore: false`
    // forbids the self-heal path. Either way, user work is protected from
    // silent drops, and the `git add -A` path cannot hang indefinitely.
