...
test("#2939: closeoutUnit call is guarded by if (s.currentUnit)", () => {
...
test("#2939: zero-tool-call guard uses s.currentUnit?.startedAt", () => {
...
test("#2939: no non-null assertions on s.currentUnit.startedAt after closeout", () => {
  ...
  const closeoutIdx = phasesSrc.indexOf("Immediate unit closeout");
  assert.ok(closeoutIdx > 0, "phases.ts contains the 'Immediate unit closeout' comment block");
...
test("#2939: final return uses s.currentUnit?.startedAt with optional chaining", () => {
