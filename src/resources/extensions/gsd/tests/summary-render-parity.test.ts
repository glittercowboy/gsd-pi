...
test("#2720: renderSummaryContent includes Verification section", () => {
...
test("#2720: renderSummaryContent includes Verification Evidence table", () => {
...
test("#2720: renderSummaryContent includes Files Created/Modified section", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
  assert.ok(
...
test("#2720: one_liner renders as bold (not blockquote) for consistency", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
  assert.ok(
...
test("#2720: frontmatter key_files uses YAML list format (not JSON array)", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
  assert.ok(
...
test("#2720: frontmatter key_decisions uses YAML list format (not JSON array)", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
  assert.ok(
...
test("#2720: Deviations section always present (with None. fallback)", () => {
...
test("#2720: Known Issues section always present (with None. fallback)", () => {
...
test("#2720: verification_result frontmatter not double-quoted", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
  assert.ok(
...
test("#2720: duration frontmatter not double-quoted", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
  assert.ok(
...
test("#2720: empty key_files renders YAML placeholder, not empty array", () => {
...
test("#2720: frontmatter does not contain extra projection-only fields", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID);
...
test("#2720: no verification evidence renders placeholder row", () => {
  const output = renderSummaryContent(taskRow, SLICE_ID, MILESTONE_ID, []);
  assert.ok(
