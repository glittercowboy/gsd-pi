...
test("#2533: missing DB slices are reconciled from disk ROADMAP", async () => {
  ...
  try {
    ...
    if (s01) {
      assert.strictEqual(s01.status, "complete", "S01 should be 'complete' (has SUMMARY on disk)");
    }

    const s02 = dbSlices.find(s => s.id === "S02");
    assert.ok(s02 !== undefined, "S02 should exist in DB after reconciliation");
    if (s02) {
      assert.strictEqual(s02.status, "complete", "S02 should be 'complete' (has SUMMARY on disk)");
    }
    ...
    if (s03) {
      assert.strictEqual(s03.status, "pending", "S03 should be 'pending' (no SUMMARY on disk)");
    }
...
test("#2533: slice reconciliation is idempotent", async () => {
  const base = createFixtureBase();
  const dbPath = join(base, ".gsd", "gsd.db");
  ...
  try {
    ...
    if (s01) {
      assert.strictEqual(s01.status, "complete", "S01 status should remain 'complete' (not overwritten)");
    }
...
test("#2533: no ROADMAP file skips slice reconciliation", async () => {
  const base = createFixtureBase();
  const dbPath = join(base, ".gsd", "gsd.db");
  ...
  try {
    ...
    const dbSlices = getMilestoneSlices("M001");
    assert.strictEqual(dbSlices.length, 0, "No slices should be added when no ROADMAP exists");

    // Should be in pre-planning (no roadmap)
    assert.strictEqual(state.phase, "pre-planning", "Phase should be pre-planning with no roadmap");
  } finally {
    closeDatabase();
    cleanup(base);
  }
