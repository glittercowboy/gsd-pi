...
13. Update `.gsd/PROJECT.md` if it exists: use the `write` tool with `path: ".gsd/PROJECT.md"` and `content` containing the full refreshed document. Do not guess with `edit` here — if the project state needs to be refreshed, rewrite the full file through `write`.
