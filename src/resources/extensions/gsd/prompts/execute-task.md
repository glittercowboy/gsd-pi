...
16. Use the **Task Summary** output template from the inlined templates below
17. Write `{{taskSummaryPath}}`
18. Mark {{taskId}} done in `{{planPath}}` (change `[ ]` to `[x]`)
19. Do not commit manually — the system auto-commits your changes after this unit completes.
20. Update `.gsd/STATE.md`
