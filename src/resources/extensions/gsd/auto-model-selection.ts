...
export async function selectAndApplyModel(
  ...
  return withSpan("gsd.unit.model_selection", async (span) => {
    span.setAttributes({
    ...
    span.setAttribute(GSD.MODEL_CONFIGURED, modelConfig?.primary ?? "none");
    ...
    if (modelConfig) {
      ...
      if (routingConfig.enabled) {
        ...
        if (shouldClassify) {
          ...
          // OTel: record routing decision details
          span.setAttributes({
            [GSD.COMPLEXITY_TIER]: classification.tier,
            [GSD.COMPLEXITY_REASON]: classification.reason,
            [GSD.ROUTING_ENABLED]: true,
            [GSD.ROUTING_METHOD]: routingResult.selectionMethod ?? "tier-only",
            [GSD.MODEL_DOWNGRADED]: routingResult.wasDowngraded,
          });
        }
      }
      ...
      for (const modelId of modelsToTry) {
        ...
        if (ok) {
          appliedModel = model;
          span.setAttribute(GSD.MODEL_FALLBACK_USED, modelId !== effectiveModelConfig.primary);
          if (verbose) {
    ...
    span.setAttributes({
      [GSD.MODEL_RESOLVED]: appliedModel ? `${appliedModel.provider}/${appliedModel.id}` : "none",
      [GSD.MODEL_PROVIDER]: appliedModel?.provider ?? "none",
      [GSD.MODEL_DOWNGRADED]: routing?.modelDowngraded ?? false,
      [GSD.COMPLEXITY_TIER]: routing?.tier ?? "none",
    });
    return { routing, appliedModel };
  }); // withSpan("gsd.unit.model_selection")
}
