...
export async function checkRuntimeHealth(
  ...
  // (Drift detection removed per D-01 — engine is authoritative, projections are disposable)
  try {
    const stateFilePath = resolveGsdRootFile(basePath, "STATE");
    const milestonesPath = milestonesDir(basePath);

    if (existsSync(milestonesPath) && !existsSync(stateFilePath)) {
      issues.push({
        severity: "warning",
...
// DB constraint violation detection + projection drift repair (D-08, D-09, D-10)
...
export async function checkEngineHealth(
  ...
  // ── Projection drift detection (also runs in pre-dispatch per D-10) ──
  try {
    if (isEngineAvailable(basePath)) {
