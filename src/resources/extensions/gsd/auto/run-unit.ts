...
import { withSpan, GSD, SpanStatusCode } from "../tracing/index.js";
...
export async function runUnit(
  ...
  return withSpan("gsd.unit.execute", async (span) => {
    span.setAttributes({
    ...
    span.setAttribute(GSD.UNIT_RESULT, result.status);
    if (result.status === "error") {
      span.setStatus({ code: SpanStatusCode.ERROR });
    }
    return result;
  }); // withSpan("gsd.unit.execute")
}
