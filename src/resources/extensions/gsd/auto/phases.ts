...
export async function runPreDispatch(
  ...
  return withSpan("gsd.pre_dispatch", async (span) => {
    ...
    span.setAttributes({
      [GSD.STATE_PHASE]: state.phase,
      [GSD.STATE_MILESTONE]: mid,
      [GSD.STATE_SLICE]: state.activeSlice?.id ?? "none",
      [GSD.STATE_TASK]: state.activeTask?.id ?? "none",
    });
    if (state.blockers.length > 0) {
      span.setAttribute(GSD.STATE_BLOCKERS, state.blockers.join(", "));
    }
    return { action: "next", data: { state, mid, midTitle } };
  }); // withSpan("gsd.pre_dispatch")
}
...
export async function runDispatch(
  ...
  return withSpan("gsd.dispatch", async (span) => {
    const { ctx, pi, s, deps, prefs } = ic;
    ...
    span.setAttributes({
      [GSD.DISPATCH_UNIT_TYPE]: unitType,
      [GSD.DISPATCH_UNIT_ID]: unitId,
      [GSD.DISPATCH_RULE]: dispatchResult.matchedRule ?? "unknown",
      [GSD.DISPATCH_ACTION]: "dispatch",
    });
    return {
      action: "next",
      data: {
...
export async function runGuards(
  ...
  return withSpan("gsd.guards", async (span) => {
    const { ctx, pi, s, deps, prefs } = ic;
    ...
    if (budgetCeiling !== undefined && budgetCeiling > 0) {
      ...
      span.setAttributes({
        [GSD.BUDGET_PCT]: Math.round(budgetPct * 100),
        [GSD.BUDGET_ENFORCEMENT]: prefs?.budget_enforcement ?? "pause",
      });
...
export async function runUnitPhase(
  ...
  return withSpan("gsd.unit", async (span) => {
    const { ctx, pi, s, deps, prefs } = ic;
    ...
    span.setAttributes({
      [GSD.UNIT_RESULT]: unitResult.status,
      [GSD.UNIT_DURATION_MS]: s.currentUnit ? Date.now() - s.currentUnit.startedAt : 0,
      [GSD.UNIT_RETRY]: isRetry,
      [GSD.UNIT_DISPATCH_COUNT]: s.unitDispatchCount.get(`${unitType}/${unitId}`) ?? 1,
    });
    return { action: "next", data: { unitStartedAt: s.currentUnit?.startedAt } };
  }); // withSpan("gsd.unit")
}
...
export async function runFinalize(
  ...
  return withSpan("gsd.finalize", async (span) => {
